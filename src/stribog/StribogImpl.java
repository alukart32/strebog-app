...
public class StribogImpl implements Stribog {
  ...
  private final byte[][] A = {
      ...
      {(byte) 0x64, (byte) 0x1c, (byte) 0x31, (byte) 0x4b, (byte) 0x2b, (byte) 0x8e, (byte) 0xe0, (byte) 0x83}
  };
   ...
   * Инициализация полей
   */
  protected void init(boolean outputSize) {
    h = new byte[64];
    if (!outputSize) {
  ...
  public byte[] getHash(byte[] msg, boolean outputSize) {
    init(outputSize);
    return generateHash(msg, outputSize);
  }

  /**
   * Обрабатывает блок данных
   *
   * @param msgChunk блок данных 512 бит или меньше
   */
  protected void hashPart(byte[] msgChunk) {
    if (msgChunk.length == 64) {
      h = g(N, msgChunk, h);
      N = or(N, Vector512);
      sigma = or(sigma, msgChunk);
    } else {
      int inc = 0;
      int length = msgChunk.length * 8;

      //Дополнение до 512 бит по алгоритму
      byte[] tmpMessage = new byte[64];
      Arrays.fill(tmpMessage, (byte) 0x00);
      tmpMessage[64 - msgChunk.length - 1] = (byte) 0x01;

      for (int i = 64 - msgChunk.length; i < 64; i++) {
        tmpMessage[i] = msgChunk[i + msgChunk.length - 64];
      }
   ...
   */
  @Deprecated
  private byte[] generateHash(byte[] msg, boolean outputMode) {
    int length = msg.length * 8;
    int inc = 0;

    while (length >= 512) {
      inc++;
      byte[] tmp = Arrays.copyOfRange(msg, msg.length - 64 * (inc), msg.length - (inc - 1) * 64);
      h = g(N, tmp, h);
      N = or(N, Vector512);
      sigma = or(sigma, tmp);
      length -= 512;
    ...
    msg = Arrays.copyOf(msg, msg.length - 64 * inc);

    if (msg.length < 64) //обработка сообщения длинной < 512 бит
    {
      byte[] tmpMsg = new byte[64];
      Arrays.fill(tmpMsg, (byte) 0x00);
      tmpMsg[64 - msg.length - 1] = (byte) 0x01;

      for (int i = 64 - msg.length; i < 64; i++) {
        tmpMsg[i] = msg[i + msg.length - 64];
      }
   ...
   * Значение хэш-кода сообщения вычисляется с использованием итерационной процедуры. На каждой итерации вычисления
   * хэш-кода используется функция сжатия: V512 x V512 -> V512
   *
   * @param N вектор инициализации
   * @param m блок данных
   * @param h часть хеша
   */
  private byte[] g(byte[] N, byte[] m, byte[] h) {
  ...
  /**
   * Часть функции сжатия. Состоит из 12 раундов. Фактически блочный шифр.
   *
